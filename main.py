print("=== ЗАПУСК СКРИПТА ВЕРСИИ 5.4 (RU_FOCUS_DEDUP_CONTEXT) ===")
...
# 3. Пре-фильтр таблоидного шума и не относящихся к делу тем.
# Отсекается ДО отправки в LLM — экономит токены и не даёт модели
# случайно "спасти" такую новость в саммари.
JUNK_KEYWORDS = [
...
# Модель не всегда надёжно держит инструкцию из промпта, поэтому те же
# правила дополнительно проверяются на выходе по ключевым словам/именам.
...
def generate_analytical_json(raw_data_prompt):
    ...
    3. Выдерживай международный баланс (не менее 40% зарубежных медиа), НО соблюдай фильтр значимости.
    ...
    5. КАТЕГОРИЧЕСКИ ИСКЛЮЧАЙ ЛЮБУЮ ЛОКАЛЬНУЮ/РЕГИОНАЛЬНУЮ ВНУТРЕННЮЮ ПОЛИТИКУ ЛЮБОЙ СТРАНЫ
       ...
       - Примеры того, что нужно исключать (это ТОЛЬКО иллюстрация принципа, а не полный список —
         исключай ЛЮБУЮ новость такого же типа, даже если её нет в примерах): праймериз в отдельном
         штате США, борьба конкретного политика за место в истории партии, назначение отдельного
         судебного чиновника, региональная инициатива местных властей.
       ...
       - Если сомневаешься, национальный это уровень или локальный — ИСКЛЮЧАЙ.

    6. ОБЪЕДИНЯЙ ДУБЛИКАТЫ: если несколько ID описывают ОДНО И ТО ЖЕ событие (например, одно и то же
       землетрясение, одно и то же решение ЦБ, одну и ту же сделку — из разных источников), выбери
       ТОЛЬКО ОДИН ID (предпочтительно — источник с наиболее содержательным контекстом) и включи
       событие только ОДИН РАЗ, ТОЛЬКО в одну наиболее подходящую категорию. Не дублируй событие
       ни внутри категории, ни между категориями.

    7. КАТЕГОРИЧЕСКИ ИСКЛЮЧАЙ: бытовую недвижимость, ремонт дорог, спорт, шоу-бизнес, бытовые ДТП,
       погоду как таковую (кроме случаев, когда это значимый экономический/политический фактор),
       любые новости, где суть — курьёзный или развлекательный факт, а не решение/тенденция.

    8. "summary_ru" — не просто факт, а факт + краткий контекст: почему это происходит и/или
       чем это важно (какие последствия, чьи интересы затронуты, в какую тенденцию укладывается).
       Формат: "Факт. Почему/зачем: контекст." Уложись в 220 символов, без скобок и без названий
       источников внутри текста. Но не пиши в начале каждой новости "Факт:"

    9. Переводи всё на русский язык.
...
def build_html_digest(raw_response, news_db):
    ...
    seen_urls_in_digest = set()  # страховка от дублей одного события в разных категориях
    html_output = ""
