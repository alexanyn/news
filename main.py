print("=== ЗАПУСК СКРИПТА ВЕРСИИ 5.6 (STRICT_SLOTS_AND_PR_FILTER) ===")
...
def generate_analytical_json(raw_data_prompt):
    ...
    2. ИЗ-ЗА ЛИМИТА ТЫ ОБЯЗАН ОБЪЕДИНЯТЬ ДУБЛИКАТЫ: если разные источники пишут про одно и то же (например, снятие "Яблока" с выборов), выбери ТОЛЬКО ОДИН ID, самый содержательный. Не трать слоты рубрики на дубли!
    3. Поле "id" ДОЛЖНО СТРОГО СОВПАДАТЬ с ID из входящего блока.
    4. Поле "source_name" должно совпадать с источником под этим ID.
    5. РОССИЯ — ПРИОРИТЕТ, ставь её первыми пунктами в категории.
    6. ВЗАИМОИСКЛЮЧЕНИЕ КАТЕГОРИЙ: Каждый ID может быть использован строго в ОДНОЙ категории.
    7. КАТЕГОРИЧЕСКИ ИСКЛЮЧАЙ мусор: 
       - Локальную внутреннюю политику (праймериз, назначения мэров).
       - Местечковый корпоративный PR (открытия отдельных магазинов "Бекон", новые филиалы, мелкие запуски продуктов).
