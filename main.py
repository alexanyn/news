print("=== ЗАПУСК СКРИПТА ВЕРСИИ 6.20 (COMPANY_SIGNIFICANCE_FILTER) ===")
...
#
# ИСПРАВЛЕНО 2026-08-21: переход с 3 дайджестов в сутки (morning/afternoon/
# evening — 08:00/13:00/19:00) на 5 дайджестов (09:00/12:00/15:00/18:00/21:00),
# по запросу пользователя. Имена расписаний ПЕРЕИМЕНОВАНЫ из семантических
# ("morning"/"evening") в буквальные ("09:00", "21:00" и т.д.) — при 5 слотах
# семантические имена начинают путаться (18:00 и 21:00 по-русски оба
# "вечер", 12:00 и 15:00 оба "день"), а имя расписания — это же значение,
# которое пользователь вручную вводит в 5 отдельных cron-job.org задачах;
# ошибка в одном слове там молча даёт неверное окно сбора. Буквальное имя
# исключает эту возможность: что вводишь в cron-job.org, то и есть ключ.
SCHEDULES = {
...
#
# ОБНОВЛЕНО 2026-08-21: теперь 5 элементов вместо 3 (см. SCHEDULES выше).
# get_previous_schedule_name ничего не знает о конкретном количестве
# элементов — работает через SCHEDULE_ORDER.index() и отрицательную
# индексацию Python (которая корректно оборачивается на последний элемент
# при индексе -1), поэтому сам код НЕ пришлось менять для перехода с 3 на 5 —
# достаточно было обновить только этот список и SCHEDULES выше.
...
def get_previous_schedule_name(schedule_name):
    ...
    в суточном цикле: 21:00 -> 18:00 -> 15:00 -> 12:00 -> 09:00 -> 21:00
    ПРОШЛОГО дня (Python корректно даёт последний элемент списка при
    индексе -1, поэтому переход через полночь работает без отдельного
    случая для этого)."""
...
def get_schedule_boundary(schedule_name):
    ...
    ПРИМЕЧАНИЕ 2026-08-21: пример выше ("morning", 08:00) — реальные имя и
    время из инцидента, который этот фикс устранил, оставлены как есть для
    точности истории. Сами имена расписаний с тех пор переименованы (см.
    SCHEDULES выше — теперь "09:00"/"12:00"/"15:00"/"18:00"/"21:00" вместо
    "morning"/"afternoon"/"evening"), но описанная здесь логика и причина
    бага не изменились — тот же механизм применим к любому имени расписания.
    """
...
def get_next_publish_time(schedule_name):
    ...
    ДОЖДАТЬСЯ перед фактической отправкой в Telegram, чтобы дайджест не
    публиковался сразу после сбора (что и вызывало эффект "дайджест выходит через
...
def collect_all_news(sent_urls_history, recent_titles_history, schedule_name="09:00"):
    ...
    schedule_name: одно из SCHEDULES — "09:00", "12:00", "15:00", "18:00", "21:00"
    ...
    Это позволяет собирать новости за нужный промежуток (09:00–12:00, 12:00–15:00
    и т.д. — все окна между соседними по хронологии расписаниями, кроме
    09:00, чья граница отсчитывается от 21:00 ПРОШЛОГО дня, см. ниже),
    ...
    # дайджеста (например, для 12:00 это 09:00, для 09:00 — 21:00 ПРОШЛОГО
    # дня — см. SCHEDULE_ORDER и get_previous_schedule_name). Окно сбора
    # становится узким и предсказуемым (обычно 3 часа между соседними
    # дневными расписаниями, ~12 часов для ночного окна 21:00→09:00 — см.
    # ОБНОВЛЕНО 2026-08-21 ниже про переход на 5 расписаний в сутки), не
    # зависит от того, запускался ли этот же дайджест сегодня повторно, и не
    # полагается на sent_urls_history как на единственную линию защиты от
    # дублей (хотя она остаётся полезной подстраховкой на случай пересечения
    # окон при ручных перезапусках).
    #
    # ОБНОВЛЕНО 2026-08-21: переход с 3 дайджестов в сутки (окна ~5-6 часов)
    # на 5 дайджестов (09:00/12:00/15:00/18:00/21:00) сузил дневные окна
    # примерно вдвое — до ~3 часов каждое (09→12, 12→15, 15→18, 18→21),
    # кроме ночного окна 21:00(вчера)→09:00(сегодня), которое осталось
    # широким (~12 часов). Более узкие окна означают МЕНЬШЕ сырых кандидатов
    # на дневной прогон при том же наборе из 104 источников — это стоит
    # учитывать, если объём новостей в дневных дайджестах покажется меньше,
    # чем ожидалось после недавнего расширения потолка до 7 новостей на
    # категорию (см. промпт в generate_analytical_json) — потолок не
    # изменился, изменилось количество материала, из которого есть выбор.
...
def generate_analytical_json(raw_data_prompt, recently_published_titles=None):
    ...
    одну строку, а 2-4 предложения, которые по возможности включают:
      ...
      3) конкретное следствие — ЕСЛИ оно обоснованно следует из данных.
    ...
    которые не являются общеизвестными.

    ЗАПРЕЩЕНО (уточнено 2026-08-21, по итогам обратной связи от читателя):
    заканчивать summary_ru шаблонной фразой-выводом ОБЩЕГО характера — той,
    которую технически можно приклеить почти к любой новости без потери
    смысла, потому что она не утверждает ничего конкретного. Примеры РЕАЛЬНО
    встречавшихся запрещённых формулировок (важен сам ПАТТЕРН, не только эти
    точные слова):
      - "Данная тенденция указывает на..."
      - "Это выступление станет ключевым ориентиром..."
      - "Партнёрство позволит сочетать..."
      - "Событие подчёркивает лидерство..."
      - "Инцидент подсветил риски..."
    Общий признак таких фраз — глагол-хедж ("указывает", "подчёркивает",
    "подсвечивает", "станет ориентиром", "позволит сочетать" и т.п.) в связке
    с абстрактным объектом без конкретики ("риски" без уточнения какие,
    "лидерство" без уточнения в чём именно, "тенденция" без цифр). Третье
    предложение (см. пункт 3 выше) допустимо ТОЛЬКО если называет что-то
    КОНКРЕТНОЕ: точную цифру, конкретного названного актора, которого это
    затронет, конкретный механизм или ближайший ожидаемый шаг — то есть
    утверждение, которое в принципе можно было бы проверить и которое
    оказалось бы неверным при других обстоятельствах. Если из данных не
    выводится ничего настолько конкретного — ЛУЧШЕ ЗАКОНЧИТЬ summary_ru на
    факте и контексте (1-2 предложения), чем добавлять пустое по содержанию
    третье предложение. Точность и конкретность важнее наличия "аналитично
    звучащего" предложения любой ценой.
    ...
    4. Для категорий technology и business — рядовые корпоративные пресс-релизы
       ...
       ФИЛЬТР ПО ЗНАЧИМОСТИ КОМПАНИИ (добавлено 2026-08-28): наличие масштаба
       в новости (сумма сделки, оценка, % движения акций) само по себе НЕ
       гарантирует включение — типичный случай, который упускало правило выше:
       "стартап X привлёк $5 млн" технически содержит масштаб (сумму), но это
       всё равно рядовая новость, если сам стартап незначителен. Для technology
       и business (НЕ для pr — там своё, более мягкое правило ниже) новость о
       конкретной компании (назначения, оценки/капитализация, привлечение
       финансирования, движения акций и любые другие корпоративные новости)
       включай, только если верно ХОТЯ БЫ ОДНО из условий:
       а) компания объективно крупная — по капитализации, выручке, месту в
          мировых рейтингах (не обязательно на слуху у широкой публики,
          достаточно быть крупной по факту — например, промышленный
          B2B-поставщик с многомиллиардной выручкой, о котором мало кто
          слышал за пределами отрасли, подходит);
       б) компания широко узнаваема — практически все о ней слышали, даже
          если формально не входит в мировые финансовые топы.
       (а) и (б) равнозначны — достаточно ЛЮБОГО ОДНОГО из двух, оба сразу
       не требуются.
       в) для РОССИЙСКИХ компаний — отдельная, более мягкая планка: топ своей
          отрасли в России достаточно, даже если по мировым меркам не гигант
          (ориентир по уровню: Сбербанк, Яндекс, Т-Банк, Газпром, Лукойл,
          Норникель, ВК и сопоставимые лидеры российских отраслей). Некрупные
          и малоизвестные даже в российском контексте компании — исключай.
       г) НЕЗАВИСИМО от размера/узнаваемости компании — если само СОБЫТИЕ
          значимо для мира или отрасли целиком: прорывная технология или
          инновация (даже у небольшой малоизвестной компании — например,
          первый коммерческий термоядерный реактор с устойчивым положительным
          энергобалансом) ИЛИ событие двигает всю индустрию/рынок, а не
          только эту одну компанию (например, новое регулирование,
          затрагивающее всех игроков отрасли, даже если озвучено через
          небольшую компанию).
       Примеры (мир): ОСТАВЛЯЙ "Nvidia отчиталась о рекордной выручке",
       "Акции Tesla упали на 12% после отчёта" — крупные и узнаваемые
       компании. ИСКЛЮЧАЙ "Вице-президентом по маркетингу малоизвестного
       стартапа X назначен Y", "Стартап Z привлёк $5 млн на pre-seed раунде"
       — компания не проходит ни один из критериев выше, само наличие суммы
       в новости не спасает.
       Примеры (Россия): ОСТАВЛЯЙ "Сбербанк объявил о смене топ-менеджмента"
       — топ российская компания. ИСКЛЮЧАЙ "Региональная компания из
       Екатеринбурга привлекла раунд финансирования" — не топ-уровень даже
       с указанной суммой.
...
def build_html_digest(raw_response, news_db):
    ...
    def build_one(target_is_russia, header, sections_to_render, force_show=False):
        ...
        for key, title in sections_to_render:
            ...
            # ИСПРАВЛЕНО 2026-08-21: заголовок подкатегории рендерится только
            # если секций в этом блоке больше одной. Для MAIN_SECTIONS (6
            # категорий) это не меняет поведение — там подзаголовки нужны,
            # чтобы отличить "🌍 ГЕОПОЛИТИКА" от "📈 ЭКОНОМИКА" и т.д. Но для
            # PR_SECTIONS (всего 1 категория) заголовок региона ("📢 PR В
            # МИРЕ") и заголовок этой единственной категории ("📢 PR И
            # КОММУНИКАЦИИ") дублировали друг друга визуально — пользователь
            # поймал это на реальном дайджесте (два подряд "PR"-заголовка
            # без новостей между ними). При единственной секции сразу
            # переходим к пунктам/плейсхолдеру под заголовком региона.
...
if __name__ == "__main__":
    ...
    # Использование: python main.py 09:00 / python main.py 12:00 / и т.д.
    # (полный список допустимых значений — ключи SCHEDULES, см. выше)
    schedule_name = "09:00"  # default
    if len(sys.argv) > 1:
